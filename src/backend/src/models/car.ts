...
export const createCar = async (input: CarInput): Promise<Car> => {
  const pool = database.getPool();
  const result = await pool.query(
    `INSERT INTO cars (make, model, version, year, mileage, fuel, transmission, power, displacement, color, doors, seats, body_type, price, description, features, images, sold, featured)
